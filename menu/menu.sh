...
Exp=$(curl -sS https://raw.githubusercontent.com/GeKaStore/izin/main/izin | grep $MYIP | awk '{print $3}')
fi
...
Exp1=$(curl https://raw.githubusercontent.com/GeKaStore/izin/main/izin | grep $MYIP | awk '{print $4}')
if [[ $today < $Exp1 ]]; then
sts="${Info}"
else
sts="${Error}"
