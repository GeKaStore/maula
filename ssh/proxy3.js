...
function gcollector() {
  if (!global.gc && gcwarn) {
    console.log(
      "[WARNING] Garbage Collector isn't enabled! Memory leaks may occur."
    );
    gcwarn = false;
    return;
  } else if (global.gc) {
...
server.on("connection", function (socket) {
  ...
  socket.write(
    "HTTP/1.1 101 Autoscript By WUZZSTORE TUNNEL\r\nContent-Length: 1048576000000\r\n\r\n",
    function (err) {
      if (err) {
        console.log(
          "[SWRITE] Failed to write response to " +
  ...
  conn.on("data", function (data) {
    ...
    socket.write(data, function (err) {
      if (err) {
        console.log(
          "[SWRITE2] Failed to write response to " +
            socket.remoteAddress +
            ":" +
            socket.remotePort +
            ", error: " +
